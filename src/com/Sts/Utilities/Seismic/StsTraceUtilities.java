     ...
     */
     static public float[] computeCubicInterpolatedPoints(float[] inputPoints, int nIntervals)
     {
         ...
         try
         {
			 ...
			 double len1 = Math.sqrt(dz1*dz1 + 1);
			 double len2 = Math.sqrt(dz2*dz2 + 1);
			 double slope1 = dz1;
			 double f = len1/(len1 + len2);
			 double slope2 = dz1 + f*(dz2 - dz1);
			 ...
			 for (int i = 3; i < nInputPoints; i++)
             {
				 ...
				 len1 = len2;
				 len2 = Math.sqrt(dz2*dz2 + 1);

				 slope1 = slope2;
				 f = len1/(len1 + len2);
				 slope2 = dz1 + f*(dz2 - dz1);
				 //slope1 = 0.5 * (z2 - z0);
				 //slope2 = 0.5 * (z3 - z1);
	 ...
	 {
		 return pointType >= POINT_PLUS_ZERO_CROSSING && pointType <= POINT_FALSE_MINIMUM;
	 }
